...
try {
    ...
    foreach($remoteRecords as $record) {
        ...
        if ($exists['cnt'] == 0) {
            ...
            // Changed $by_user from 'remote_sync' (string) to 0 (integer) to match column type.
            $by_user    = 0;
            ...
            $stmtInsert = $pdo->prepare("INSERT INTO accounts (by_user, aws_key, aws_secret, account_id, status, added_date) VALUES (?, ?, ?, ?, ?, ?)");
            if ($stmtInsert->execute([$by_user, $aws_key, $aws_secret, $account_id, $status, $added_date])) {
                $newRecordsCount++;
            }
        }
    }
